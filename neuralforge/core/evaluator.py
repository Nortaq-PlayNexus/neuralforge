...
class ModelEvaluator:
    ...
    def evaluate(self, model_name: str, dataset_path: str, metrics: list[str]) -> dict:
        ...
        try:
            ...
            for i, sample in enumerate(dataset[:100]):
                ...
                with torch.no_grad():
                    outputs = model.generate(**inputs, max_new_tokens=256, temperature=0.7, return_dict_in_generate=True)
    ...
    def compare_models(self, model_a: str, model_b: str, dataset_path: str, metrics: list[str] | None = None) -> dict:
        if metrics is None:
    ...
    def _compute_f1(self, predictions: list[dict]) -> dict:
        ...
        return {"value": round(f1, 4), "precision": round(precision, 4), "recall": round(recall, 4)}
    ...
    def _compute_confusion_matrix(self, predictions: list[dict]) -> dict:
        ...
        labels = sorted(set(p["expected"] for p in predictions) | set(p["predicted"] for p in predictions))
